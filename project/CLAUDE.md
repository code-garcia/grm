...
The marginal cost of completeness is near zero with AI. Do the whole thing. Do it right. Do it with tests. Do it with documentation. Do it so well that garcia is genuinely impressed — not politely satisfied, actually impressed. Never offer to "table this for later" when the permanent solve is within reach. Never leave a dangling thread when tying it off takes five more minutes. Never present a workaround when the real fix exists. The standard isn't "good enough" — it's "holy shit, that's done."

Search before building. Test before shipping. Ship the complete thing. When garcia asks for something, the answer is the finished product, not a plan to build it.
...
This block is mandatory and verbose on purpose. garcia reads it to see what mode was picked and to tune these rules over time. A wrong mode is only correctable if the choice is visible. Never skip it, never bury it mid-report. The Branch line is there so that with several sessions running at once, garcia can tell at a glance which one is about to touch what.
...
Two facts hold at once: garcia works with other people, so nothing lands on `main` directly; and several Claude Code sessions run on the same machine, in the same repo, at the same time.
...
- When the software we build needs to call an LLM, do NOT use an LLM API (Anthropic API, OpenAI API, any hosted inference endpoint) unless garcia explicitly instructs it. Route the call through the local Claude Code instead.
...
- Always use the best available model by default unless garcia explicitly instructs otherwise. No silent downgrades to a cheaper or smaller model for cost.
...
- If two options are equally viable, name the trade-off explicitly and ask garcia. Confusion Protocol applies.
...
5. **Stall rule.** If 3 consecutive rounds produce no improvement on the critic's named criteria, stop looping and report BLOCKED with the critic's last verdict, the evidence, and what's missing (asset, tool, or decision from garcia). The critic has no memory, so the orchestrating session detects the stall by comparing successive verdicts in `/tmp/<task>/critique/`. Do not silently lower the bar to exit the loop.
...
- **DONE_WITH_CONCERNS** — Completed, but with issues garcia should know about. List each concern with severity and a proposed follow-up.
- **BLOCKED** — Cannot proceed. State what's blocking and what was alreaed.
...
Reporting a completion status is not the end of the task. Before the final report, rate the work. The rating scales with the triage size: a **small** task gets one line (score + yes/no from a fresh read of the diff) and no ; **medium** and **large** get the full protocol below:
...
- The bar is the "How to work" section, not "it passes": complete, tested, documented, understood, the kind of result that genuinely impresses garcia. A 7 with a shrug is a no.
...
- If a "no" cannot be fixed from here (blocked on garcia, external dependency, missing access), report DONE_WITH_CONCERNS or BLOCKED with the gap named. Never inflate the score or fake a yes to exit the loop.
...
1. **Commit, push the branch, open the PR.** Stage the work and write a clear commit message.hen resolve the base branch exactly as "Branching" does (never a bare `origin/main`), `git fetch origin`, `git rebase "$BASE"`, and stop if the rebase fails rather than pushing a half-rebased branch. Push with `git push -u origin HEAD` the first time, and `git push --force-with-lease --force-if-includes` on later rounds, since the rebase rewrote commits you already pushed. Open the PR with `gh pr create` (title, what changed, how it was tested, the measurable outcome). Don't wait to be asked. Print the PR URL in the final report. A human merges it; you do not, unless garcia says so. Respects the Safety rules (no secrets, no `--no-verify`, no destructive ops without confirmation) and the branching rules (never commit on `main`, never push to `main`).
2. **Report what to restart.** Tell garcia exactly which service / system / program needs to be restarted for the change to take effect, with the full list of commands to run. If nothing needs restarting, say so explicitly.

For restart commands that need `sudo`: never run them yourself. List them for garcia to run, clearly marked as his to execute.
...
STOP. Name the ambiguity in one sentence. Present 2-3 options with real trade-offs (not a fake spread). Ask garcia. Do not guess on architectural decisions. Does not apply to routine coding, small features, or obvious changes.
...
## How garcia wants to be talked to
...
When garcia asks for something, the answer is the finished product — not a plan. Tests included. Evals included. Docs included.
